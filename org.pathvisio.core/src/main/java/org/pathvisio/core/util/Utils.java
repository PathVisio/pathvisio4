...
public class Utils {
	...
	public static String summary(PathwayElement elt) {
		...
		StringBuilder result = new StringBuilder("[" + elt.getClass().getSimpleName());
	...
	/**
	 * Helper that returns all Lines that point to a given Anchor or Shape
	 */
	public static Set<PathwayElement> getReferringLines(LinkableTo elt) {
		Set<PathwayElement> result = new HashSet<PathwayElement>();
		for (LinkableFrom grc : elt.getLinkableFroms()) {
			if (grc instanceof LinePoint) {
				result.add(((LinePoint) grc).getLineElement());
			}
		}
		return result;
	}
