...
/**
 * Wrapper for org.pathvisio.model.Pathway that adds some extra functionality
 * for gpmldiff
 */
public class PwyDoc {
	...
	/**
	 * Construct a new PwyDoc from a certain file Returns null if there is an IO
	 * exception TODO: We may want to pass on the exception?
	 */
	...
	/**
	 * applies the cache of modified elements back to the wrapped pathway object
	 * itself
	 */
	public void apply() {
	 ...
	 */
	static public Map2D<PathwayElement, PathwayElement, Integer> getSimTable(PwyDoc oldDoc, PwyDoc newDoc,
			SimilarityFunction simFun) {
		Map2D<PathwayElement, PathwayElement, Integer> result = new Map2D<PathwayElement, PathwayElement, Integer>(
				oldDoc.getElts(), newDoc.getElts());
	 ...
	 * 
	 * Originally I planned to use Dijkstra's algorithm as described in the xmldiff
	 * whitepaper, but it turned out not to be necessary, and a simple ad-hoc
	 * algorithm suffices (and is probably faster)
	 * 
	 * Call this on the OLD doc
	 * 
	 * @param newDoc  the New doc
	 * @param simFun  similarity function
	 * @param costFun cost function (for deciding which correspondence set to
	 *                choose)
	 */
	public SearchNode findCorrespondence(PwyDoc newDoc, SimilarityFunction simFun, CostFunction costFun) {
		/*
		 * Compare each old element with each new element and store this in a scoring
		 * matrix.
		 */
		Map2D<PathwayElement, PathwayElement, Integer> scoreMatrix = getSimTable(this, newDoc, simFun);

		/*
		 * We're going to calculate the maximum score for each row in the table and
		 * store the results in this Map.
		 */
		Map<PathwayElement, Integer> maxScores = new HashMap<PathwayElement, Integer>();
		...
		/*
		 * Now sort the elements descending by their maximum score. The reasoning is
		 * that if the maximum score is high (e.g. 100), this means that there is really
		 * no doubt that they go together, so we want to get those done first and then
		 * leave the doubtful ones last.
		 * 
		 * That way we prevent that a doubtful pair gets mixed up with a perfect
		 * matching pair.
		 */
		Collections.sort(elts, new MaxScoreComparator(maxScores));

		/*
		 * The results are recorded in a linear tree of SearchNodes. This is perhaps a
		 * strange way to record the results, but it works and it will make it easier to
		 * switch to a different algorithm in the future without modifying the rest of
		 * the project.
		 */
	...
	/**
	 * Output the Diff after the best correspondence has been calculated. call this
	 * on the OLD doc.
	 * 
	 * @param result result of findcorrespondence
	 * @param newPwy the new pathway compared against
	 * @param out    DiffOutputter that absorbs the results and puts it somewhere
	 *               depending on the type of DiffOutputter
	 */
