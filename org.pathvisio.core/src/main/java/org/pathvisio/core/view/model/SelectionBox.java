 ...
 */
public class SelectionBox extends VElement implements Adjustable {
	// Corner handles
	...
	private Set<VElement> selection;
	...
	public SelectionBox(VPathwayModel canvas) {
		...
		selection = new HashSet<VElement>();
	...
	public Set<VElement> getSelection() {
		return selection;
	}
	 ...
	 */
	public VElement getChild(Point2D p) {
		// First check selection
		for (VElement o : selection) {
			if (o.vContains(p))
				return o;
		}
	...
	/**
	 * Returns true if the selectionbox has multiple objects in its selection, false
	 * otherwise
	 */
	...
	/**
	 * Resets the selectionbox (unselect selected objects, clear selection, reset
	 * rectangle to upperleft corner
	 */
	public void reset() {
		reset(0, 0, true);
	}
	...
	private void reset(double vStartX, double vStartY, boolean clearSelection) {
		if (clearSelection) {
			...
			selection.clear();
			if (hadObjects) {
				fireSelectionEvent(new SelectionEvent(this, SelectionEvent.SELECTION_CLEARED));
			}
		}
	...
	/**
	 * Returns true if this selectionbox is in selecting state (selects containing
	 * objects when resized)
	 */
...
//		deselect();
	...
	public void fitToSelection() {
		...
		if (!hasMultipleSelection()) { // Only one object in selection, hide selectionbox
			VElement passTo = selection.iterator().next();
			passTo.select();
			return;
		}
	...
	/**
	 * This method implements actions performed when the width of the object becomes
	 * negative after adjusting to a handle
	 *
	...
	public void doDraw(Graphics2D g) {
		if (isVisible) {
			int sw = 1;
			g.setStroke(
					new BasicStroke(sw, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_MITER, 1, new float[] { 1, 2 }, 0));
	...
	/**
	 * implement this if you want to be notified when the set of selected objects
	 * changes
	 */
	...
	public static class SelectionEvent extends EventObject {
		...
		public VElement affectedObject;

		public int type;

		public Set<VElement> selection;

		public SelectionEvent(SelectionBox source, int type, VElement affectedObject) {
			super(source);
			this.source = source;
			this.type = type;
	...
	public double getVWidth() {
		return vFromM(mWidth);
	}

	public double getVHeight() {
		return vFromM(mHeight);
	}
}
