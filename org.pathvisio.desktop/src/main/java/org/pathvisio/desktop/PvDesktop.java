 ...
 * access to them for the entire PathVisio standalone application. PvDesktop
 * provides access to components such as data visualization and gene expression
 * data. It is also a contact point for Plugins, and makes sure Gex data is
...
public class PvDesktop implements ApplicationEventListener, GdbEventListener {
	...
	public void initPanels() {
		preferencesDlg.addPanel("Display", preferencesDlg.builder()
				.integerField(GlobalPreference.GUI_SIDEPANEL_SIZE, "Initial side panel size (percent of window size):",
						0, 100)
				.booleanField(GlobalPreference.DATANODES_ROUNDED, "Use rounded rectangles for data nodes")
				.integerField(GlobalPreference.MAX_NR_CITATIONS, "Maximum citations to show (use -1 to show all)", -1,
						1000)
				.booleanField(GlobalPreference.SNAP_TO_ANGLE, "Snap to angle when moving line and rotation handles")
				.integerField(GlobalPreference.SNAP_TO_ANGLE_STEP, "Distance between snap-steps in degrees:", 1, 90)
				.booleanField(GlobalPreference.MIM_SUPPORT,
						"Load support for molecular interaction maps (MIM) at program start")
				.booleanField(GlobalPreference.SHOW_ADVANCED_PROPERTIES, "Show advanced properties (e.g. references)")
				.booleanField(GlobalPreference.USE_SYSTEM_LOOK_AND_FEEL,
						"Use Java System look-and-feel at program start")
				.booleanField(GlobalPreference.ENABLE_DOUBLE_BUFFERING,
		...
		preferencesDlg.addPanel("Display.Colors",
				preferencesDlg.builder()
						.colorField(GlobalPreference.COLOR_NO_CRIT_MET, "Default color for 'no criteria met':")
						.colorField(GlobalPreference.COLOR_NO_GENE_FOUND, "Default color for 'gene not found':")
						.colorField(GlobalPreference.COLOR_NO_DATA_FOUND, "Default color for 'no data found':")
						.colorField(GlobalPreference.COLOR_SELECTED, "Line color for selected objects:")
						.colorField(GlobalPreference.COLOR_HIGHLIGHTED, "Highlight color").build());

		preferencesDlg.addPanel("Files",
				preferencesDlg.builder().fileField(GlobalPreference.FILE_LOG, "Log file:", false).build());

		preferencesDlg.addPanel("Directories",
				preferencesDlg.builder().fileField(GlobalPreference.DIR_PWFILES, "Gpml pathways:", true)
						.fileField(GlobalPreference.DIR_GDB, "Gene databases:", true)
						.fileField(GlobalPreference.DIR_EXPR, "Expression datasets:", true).build());

		preferencesDlg.addPanel("Database",
				preferencesDlg.builder()
						.stringField(GlobalPreference.DB_ENGINE_GEX, "Database connector class for expression dataset:")
						.build());
	 ...
	 * window. Useful for positioning / parenting dialogs
	 */
	...
	public void registerMenuAction(String submenu, Action a) {
		...
		if (menuBar == null) {
			Logger.log.warn(
					"Trying to register menu action while no menubar is available " + "(running in headless mode?)");
			return;
		}
	...
	/**
	 * should be called from done method in the plugin class so all created submenus
	 * are removed
	 */
	...
	/**
	 * Returns the JTabbedPane that corresponds to the side-bar shortcut for
	...
	private void initPluginPreference() {
		...
		dlg.addPanel("Plugin Repository",
				dlg.builder().stringField(PluginRepoPreference.ONLINE_REPO_URL, "Online repository URL").build());
	}

	/**
	 * plugin manager registers a service in the activator this methods gets the
	 * plugin manager class from the OSGi registry
	 * 
	 * @return
	 */
